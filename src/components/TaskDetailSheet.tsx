...
import { Calendar, Paperclip, MessageSquare, Trash2, User, Maximize2, Minimize2, Expand, ScrollText, Send, Loader2 } from "lucide-react";
            ...
            <div className="space-y-8 animate-in fade-in duration-200">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-3 items-center">
                  ...
                  <div
                    ref={commentsRef}
                    className="flex-1 group relative flex items-center gap-2 pl-3 pr-2 py-2 rounded-2xl border border-border/40 bg-muted/20 hover:border-border/70 focus-within:border-primary/40 focus-within:bg-background transition-all duration-300"
                    onKeyDown={(e) => {
                  ...
                  >
                    <Textarea
                      ...
                      className="flex-1 border-0 bg-transparent focus-visible:ring-0 shadow-none resize-none min-h-[22px] max-h-[120px] p-0 m-0 text-sm leading-relaxed task-sheet-scroll"
                      rows={1}
                    />
                    <button
                      onClick={handleAddComment}
                      disabled={!newComment.trim()}
                      className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${newComment.trim()
                    ...
                    >
                      <Send className="h-3.5 w-3.5 transform translate-x-[1px]" />
                    </button>
                  </div>
                </div>
              </div>
